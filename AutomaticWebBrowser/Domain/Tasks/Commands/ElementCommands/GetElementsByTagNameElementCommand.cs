...
namespace AutomaticWebBrowser.Domain.Tasks.Commands.ElementCommands
{
    ...
    class GetElementsByTagNameElementCommand : ElementCommand
    {
        ...
        public GetElementsByTagNameElementCommand (IWebView webView, Logger log, AWElement element)
        ...
        public override bool Execute ()
        {
            string script = $@"
const {this.Result} = [];
function {this.Result}_ElementCommand_GetElementsByTagName_Func () {{
    const log = chrome.webview.hostObjects.log;
    try {{
        let result = this.document.getElementsByTagName ('{this.Element.Value}');
...
{this.Result}_ElementCommand_GetElementsByTagName_Func ();
".Trim ();
