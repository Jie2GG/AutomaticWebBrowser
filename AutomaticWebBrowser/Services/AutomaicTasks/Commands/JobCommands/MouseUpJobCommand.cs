    ...
    class MouseUpJobCommand : JobCommand
    {
        ...
        public override bool Execute ()
        {
            if (this.Job.Value.ValueKind == JsonValueKind.Object)
            {
                try
                {
                    ...
                    bool result = (bool)this.WebView.Invoke (() =>
                    {
                        try
                        {
                            ...
                            this.Log.Information ($"JobCommand executed “mouseUp” job of node “{this.NodeName}”, key: {(mouse.Control ? "ctrl+" : string.Empty)}{(mouse.Alt ? "alt+" : string.Empty)}{(mouse.Shift ? "shift+" : string.Empty)}{(mouse.Meta ? "meta+" : string.Empty)}, mouse: {mouse.Button}, count: {mouse.Count}.");
                            return true;
                        }
                        catch (Exception e)
                        {
                            this.Log.Error (e, $"JobCommand executed “mouseUp” job of node “{this.NodeName}”, but something went error the execution.");
                            return false;
                        }
                    });
                    return result;
                ...
                {
                    this.Log.Error (e, $"JobCommand executed “mouseUp” job of node “{this.NodeName}”, but value deserialize to “Mouse” type fail.");
                }
            }
            else
            {
                this.Log.Warning ($"JobCommand executed “mouseUp” job of node “{this.NodeName}”, but job value type is not “Mouse”.");
            }
