...
namespace AutomaticWebBrowser.Services.AutomaicTasks.Commands.JobCommands
{
    ...
    {
        public KeyUpJobCommand (BrowserForm form, GeckoNode node, Job job, Logger log)
        ...
        public override bool Execute ()
        {
            if (this.Job.Value.ValueKind == JsonValueKind.Object)
            {
                try
                {
                    ...
                    IAsyncResult asyncResult = this.Browser.BeginInvoke (new Func<bool> (() =>
                    {
                        try
                        {
                            ...
                            this.Log.Information ($"自动化任务 --> 在节点 {this.Node.NodeName} 执行 “keyUp” 作业, 键: {(keyboard.Control ? "ctrl+" : string.Empty)}{(keyboard.Alt ? "alt+" : string.Empty)}{(keyboard.Shift ? "shift+" : string.Empty)}{(keyboard.Meta ? "meta+" : string.Empty)}{keyboard.Key}, 次数: {keyboard.Count}");
                            return true;
                        }
                        catch (Exception e)
                        {
                            this.Log.Error (e, $"自动化任务 --> 在节点 {this.Node.NodeName} 执行 “keyUp” 作业, 但在执行过程中出现了异常");
                            return false;
                        }
                    }));
                ...
                {
                    this.Log.Error (e, $"自动化任务 --> 在节点 {this.Node.NodeName} 执行 “keyUp” 作业, 但作业值反序列化为 Keyboard 类型时失败");
                }
            }
            else
            {
                this.Log.Warning ($"自动化任务 --> 在节点 {this.Node.NodeName} 执行 “keyUp” 作业, 但作业值不是 Keyboard 类型");
            }
