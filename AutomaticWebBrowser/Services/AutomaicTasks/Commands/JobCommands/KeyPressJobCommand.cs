    ...
    class KeyPressJobCommand : JobCommand
    {
        ...
        public override bool Execute ()
        {
            if (this.Job.Value.ValueKind == JsonValueKind.Object)
            {
                try
                {
                    ...
                    bool result = (bool)this.WebView.Invoke (() =>
                    {
                        try
                        {
                            ...
                            this.Log.Information ($"JobCommand executed “keyPress” job of node “{this.NodeName}”, key: {(keyboard.Control ? "ctrl+" : string.Empty)}{(keyboard.Alt ? "alt+" : string.Empty)}{(keyboard.Shift ? "shift+" : string.Empty)}{(keyboard.Meta ? "meta+" : string.Empty)}{keyboard.Key}, count: {keyboard.Count}.");
                            return true;
                        }
                        catch (Exception e)
                        {
                            this.Log.Error (e, $"JobCommand executed “keyPress” job of node “{this.NodeName}”, but something went error the execution.");
                            return false;
                        }
                    });
                ...
                {
                    this.Log.Error (e, $"JobCommand executed “keyPress” job of node “{this.NodeName}”, but value deserialize to “Keyboard” type fail.");
                }
            }
            else
            {
                this.Log.Warning ($"JobCommand executed “keyPress” job of node “{this.NodeName}”, but job value type is not “Keyboard”.");
            }
