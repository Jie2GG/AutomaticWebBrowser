            ...
            IAsyncResult asyncResult = this.WebView.BeginInvoke (new Func<bool> (() =>
            {
                if (this.Job.Value.ValueKind == JsonValueKind.Object)
                {
                    try
                    {
                        ...
                        this.Log.Information ($"JobCommand executed “keyDown” job of node “{this.Node.NodeName}”, step: {Global.DOM.EVENT_KEY_DOWN}, key: {(keyboard.Control ? "ctrl+" : string.Empty)}{(keyboard.Alt ? "alt+" : string.Empty)}{(keyboard.Shift ? "shift+" : string.Empty)}{(keyboard.Meta ? "meta+" : string.Empty)}{keyboard.Key}, count: {keyboard.Count}.");
                        return true;
