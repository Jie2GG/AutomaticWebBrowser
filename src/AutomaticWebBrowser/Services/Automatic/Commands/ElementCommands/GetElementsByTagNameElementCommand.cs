...
{
    /// <summary>
    /// document.getElementsByTagName 元素查找命令
    /// </summary>
    [ElementCommand (AWElementType.GetElementsByTagName)]
    class GetElementsByTagNameElementCommand : ElementCommand
    {
        #region --构造函数--
        /// <summary>
        /// 初始化 <see cref="GetElementsByTagNameElementCommand"/> 类的新实例
        /// </summary>
        public GetElementsByTagNameElementCommand (IWebView webView, ILogger logger, AWElement element)
        ...
        /// 初始化 <see cref="GetElementsByTagNameElementCommand"/> 类的新实例
        /// </summary>
        public GetElementsByTagNameElementCommand (IWebView webView, ILogger logger, AWElement element, string? iframeVariableName)
...
(function () {{
    ...
    try {{
        let result = Array.from (this.document.getElementsByTagName ('{value}'));
    ...
    try {{
        for (let i = 0; i < {this.IframeVariableName}.length; i++) {{
            ...
            if (iframe_window != null || iframe_window != undefined){{
                let result = Array.from (iframe_window.document.getElementsByTagName ('{value}'));
                if (result != null && result != undefined) {{
                    result.forEach (element => {{
                        {this.ResultVariableName}.push (element);
                    }})
...
".Trim ();
                    ...
                    {
                        this.Logger.Information ($"自动化任务({this.WebView.TaskInfo.Name}) --> 执行 Element({this.Element.Type}) 命令失败, 原因: 未搜索到指定的元素, 值: {value}");
                    }
                }
