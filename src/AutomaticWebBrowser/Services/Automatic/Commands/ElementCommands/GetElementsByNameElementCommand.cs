...
namespace AutomaticWebBrowser.Wpf.Services.Automatic.Commands.ElementCommands
{
    ...
    class GetElementsByNameElementCommand : ElementCommand
    {
        ...
        /// </summary>
        public GetElementsByNameElementCommand (IWebView webView, ILogger logger, AWElement element)
        ...
        /// 初始化 <see cref="GetElementsByNameElementCommand"/> 类的新实例
        /// </summary>
        public GetElementsByNameElementCommand (IWebView webView, ILogger logger, AWElement element, string? iframeVariableName)
...
(function () {{
    ...
    try {{
        let result = Array.from (this.document.getElementsByName ('{value}'));
    ...
    try {{
        for (let i = 0; i < {this.IframeVariableName}.length; i++) {{
            ...
            if (iframe_window != null || iframe_window != undefined){{
                let result = Array.from (iframe_window.document.getElementsByName ('{value}'));
                if (result != null && result != undefined) {{
                    result.forEach (element => {{
                        {this.ResultVariableName}.push (element);
                    }})
